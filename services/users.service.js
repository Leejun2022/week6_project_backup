...
class UsersService {
  ...
  deleUser = async (userId) => {
    await this.userRepository.deleUser(userId);
  };
  updatUser = async (userId, nickname) => {
    const existUser = await this.usersRepository.findOneNickname(nickname);
    if (existUser) {
      throw new ValidationError("중복된 닉네임이 있습니다.");
    }
    await this.userRepository.updatUser(userId, nickname);
  };
}
